...
class Parser:
    ...
    def statement(self):
        ...
        if self.current_tok.matches(TT_KEYWORD, 'chaos_blast'):
            res.register_advancement()
            self.advance()
        ...
        if self.current_tok.matches(TT_KEYWORD, 'chaos_warp'):
            res.register_advancement()
            self.advance()
            return res.success(ContinueNode(pos_start, self.current_tok.pos_start.copy()))

        if self.current_tok.matches(TT_KEYWORD, 'chaos_control'):
            res.register_advancement()
            self.advance()
        ...
        if res.error:
            return res.failure(InvalidSyntaxError(
                self.current_tok.pos_start, self.current_tok.pos_end,
                "Expected 'var', 'if', 'for', 'while', 'create', int, float, identifier, '+', '-', '(', '[', 'chaos_blast', 'chaos_warp', 'chaos_control'"
            ))
    ...
    def atom(self):
        ...
        return res.failure(InvalidSyntaxError(
            ...
            "Expected int, float, identifier, '+', '-', '(', '[', 'if', 'for', 'while', 'create'"
        ))
    ...
    def list_expr(self):
        ...
        if self.current_tok.type == TT_RSQUARE:
            ...
            if res.error:
                return res.failure(InvalidSyntaxError(
                    self.current_tok.pos_start, self.current_tok.pos_end,
                    "Expected ']', 'var', 'if', 'for', 'while', 'create', int, float, identifier, '+', '-', '(', '['"
                ))
